...
class Agent:
    ...
    def __init__(self):
        ...
        self.model = Linear_QNet(11, 256, 3)  # 11 inputs, 256 neurons in hidden layer, 3 outputs
    ...
    def get_snake_coords(self, game):
        snake_coords = []
        for point in game.snake:
            coord = [
                point.x < game.head.x,  # is left
                point.x > game.head.x,  # is right
                point.y < game.head.y,  # is up
                point.y > game.head.y  # is down
            ]
            snake_coords.extend(coord)
        print(snake_coords)
        return np.array(snake_coords, dtype=int)

    def train_long_memory(self):
        if len(self.memory) >= BATCH_SIZE:
            memory_sample = random.sample(self.memory,
                                          BATCH_SIZE)  # randomly returns 1000 (BATCH_SIZE) elements from our memory
        else:
    ...
    def get_action(self, state):
        ...
        if random.randint(0, 200) < self.epsilon:
            ...
            state0 = torch.tensor(state,
                                  dtype=torch.float)  # converts state in a tensor (tensor = similar to multidimensional array)
            prediction = self.model(state0)
            move = torch.argmax(
                prediction).item()  # gets the index num of the maximum and converts it to an native int (e.g [5.2, 3.9, 2.4] --> 0)
            final_move[move] = 1
